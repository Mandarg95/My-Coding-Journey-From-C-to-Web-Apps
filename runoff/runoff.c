...
bool vote(int voter, int rank, string name)
{
    // Cycling throught candidates
    for (int i = 0; i < candidate_count; i++)
    {
        // Comepering names and Recording perferences
...
{
    int rank = 0;
    // Going throught the voters one by one
    for (int i = 0; i < voter_count; i++)
    {
        // Going throught candidates one by one
        for (int j = 0; j < candidate_count; j++)
        {
            // update rank if a candidates eliminated
...
bool print_winner(void)
{
    // Recording of winning vote count
    int win_votes;
    win_votes = (voter_count / 2);

    // Checking the Winner and printing it
    for (int i = 0; i < candidate_count; i++)
    {
